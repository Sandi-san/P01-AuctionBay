...
import {
  CreateBidFields,
  useCreateBidForm,
} from '../hooks/react-hook-form/useCreateUpdateBid';
...
const Auction: FC = () => {
  ...
  const calculateDate = () => {
    const timeDifference =
      new Date(item.duration).valueOf() - new Date().valueOf();
    ...
    if (timeDifference > 0) {
      ...
      if (timeRemaining < 86400) setDateClose(true);
            ...
            <div className="bg-white rounded-xl p-4 mb-4 h-1/3 flex flex-col">
                ...
                <span
                  ...
                  ${
                    auctionStatus === 'In progress'
                      ? 'bg-customYellow' // 'In progress'
                      : auctionStatus === 'Done'
                        ? 'bg-gray-800 text-white' // 'Done'
                        : auctionStatus === 'Winning'
                          ? 'bg-green-300' // 'Winning'
              ...
              {userLogged && (
                <div className="h-1/8 flex items-center justify-end mt-1">
                  <Form className="flex" onSubmit={onSubmit}>
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Form.Group className="flex">
                          <FormLabel className="mt-2" htmlFor="price">
                            Bid:
                          </FormLabel>
                          <div className="mr-2">
                            <input
                    ...
                    <Button
                      type="submit"
                      className="bg-customYellow px-4 py-1 rounded-xl "
                    >
                      Place bid
                    </Button>
                  </Form>
              ...
              {showError && (
                <ToastContainer className="" position="top-end">
                  <Toast onClose={() => setShowError(false)} show={showError}>
                    <Toast.Header>
                      <strong className="me-auto text-red-500 text-md">
                        Error
                      </strong>
                    </Toast.Header>
              ...
              {showSuccess && (
                <ToastContainer className="" position="top-end">
                  ...
                  >
                    <Toast.Header>
                      <strong className="me-auto text-green-500 text-md">
                        Success
                      </strong>
                    </Toast.Header>
                  </Toast>
                  ...
                  bids.map((bid, index) => (
                    ...
                    >
                      ...
                      <div className="flex items-center pb-1">
                        ...
                        <div className="w-10 h-10 bg-gray-300 rounded-full mr-4">
                          <Avatar
                            size="48"
                            round
                            src={`${process.env.REACT_APP_API_URL}/files/${bid.user.image}`}
                            alt="No image"
                          />
                        </div>
                      ...
                      <div className="flex items-end pb-1">
                        ...
                        {/* Bid Price */}
                        <p className="font-semibold bg-customYellow py-1 px-3 rounded-xl mb-1">
                          {bid.price}€
                        </p>
                      </div>
                    </div>
