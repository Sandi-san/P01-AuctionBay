        ...
        return (
          <div className="flex flex-col flex-grow justify-center items-center text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              ...
              <br />
              Go and bid on your favorite items!
            </p>
          </div>
        );
      default:
        return (
          <div className="flex flex-col flex-grow justify-center items-center text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              To add new auctions click "+" button in
              <br />
              navigation bar or wait for other users
              <br />
              to add new auctions.
            </p>
          </div>
        ...
        {auctions && auctions.length > 0 ? (
          <>
            <div className="flex flex-wrap p-2 bg-gray-100">
              ...
              {auctions.slice(0, 14).map(
                (
                  auction,
                  index, // Render maximum of 10 auctions
                ) => (
                  <div key={index} className="m-1">
                    {' '}
                    {/* Each auction with margin */}
                    <Card
                      item={auction}
                      user={user}
                      fetchAuctions={fetchAuctionsData}
                    />{' '}
                    {/* Render the Card component for each auction */}
                  </div>
                ),
              )}
            </div>
