...
export class AuctionController {
  ...
  async getAll(@Query('page') page: number): Promise<PaginatedResult> {
    //predenj get-as auctione, spremeni status v db ce je datum ze potekel
    this.auctionService.changeStatusByDate();
  ...
  @HttpCode(HttpStatus.OK)
  @Get(':id/bids')
  async getBidsForAuction(@Param('id') id: string): Promise<Bid[]> {
    //parsaj string iz url v int
    const auctionId = parseInt(id, 10);
    if (isNaN(auctionId)) {
      throw new BadRequestException('Invalid ID');
