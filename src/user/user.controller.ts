...
import {
  ...
  removeFile,
  saveImageToStorage,
} from 'src/helpers/image-storage.helper';
...
//(restrict-a VSE route v tem fileu)
@UseGuards(JwtGuard)
@Controller('me')
export class UserController {
  ...
  async getMe(
    //custom param decorator (glej src/auth/decorator)
  ...
  @Post('upload-image')
  //'image' mora imeti ISTO ime kot id v frontendu
  @UseInterceptors(FileInterceptor('image', saveImageToStorage))
  async uploadAvatar(
    @GetLoggedUser() user: User,
    //acutal file ki ga posiljamo
  ...
  async uploadAuctionImage(
    ...
    if (file) {
      ...
      if (auction && (await auction).image) {
        const fullImagePathToDelete = join(
          imagesFolderPath + '/' + (await auction).image,
        );
  ...
  @HttpCode(HttpStatus.OK)
  @Get('bids')
  async getBids(@GetLoggedUser() user: User): Promise<Bid[]> {
  ...
  async getAuctions(
    ...
    //klici update status Auctiona glede na duration predenj klices GET
